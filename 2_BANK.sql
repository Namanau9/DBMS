...
B)   UPDATE ACCOUNT SET BALANCE =BALANCE*1.05
     WHERE BALANCE <10000;

C) SELECT CA.ACCOUNT_NO, COUNT(CA.CUSTOMER_ID) AS NUM_CUSTOMERS
   FROM CUSTOMER_ACCOUNT CA
   GROUP BY CA.ACCOUNT_NO
   HAVING COUNT(CA.CUSTOMER_ID) > 3;

D) SELECT C.CUSTOMER_ID, SUM(0.5*BALANCE) AS TOTAL_INTEREST
   FROM CUSTOMER C, ACCOUNT A, CUSTOMER_ACCOUNT CA
   WHERE CA.ACCOUNT_NO = A.ACCOUNT_NO AND CA.CUSTOMER_ID = C.CUSTOMER_ID
   GROUP BY C.CUSTOMER_ID

E)  SELECT C.CUSTOMER_ID, C.NAME
    FROM CUSTOMER C, ACCOUNT A, CUSTOMER_ACCOUNT CA
